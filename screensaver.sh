...
#TEMPFILE is used as a lock file.  If it already exists we will assume a copy of this script is currently running and exit.
if [ -f $TEMPFILE ]; then
  exit 1
fi

#Create the TEMPFILE to act as a lock and be ready for results from the input detection processes.
touch $TEMPFILE
...
#systemd-inhibit stops the idle event that called this script from being triggered again while the screensaver is active.  It will probably block other idle triggers you have set from firing as well.  You don't need it if you depend on the conditional check for the existance of TEMPFILE at the start of this script.
systemd-inhibit inotifywait -e modify $TEMPFILE
exit
